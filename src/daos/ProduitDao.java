...
public class ProduitDao extends AbstractDao {
	...
	public static Produit insertProduit(String[] tabInfoProd, Categorie categorie, Marque marque)  {
		...
		if (nomProduit.length() <= 255) {
			...
			TypedQuery<Produit> query = em.createQuery("SELECT produit FROM Produit produit JOIN  produit.marque marque WHERE produit.nomProduit = ?1 AND marque.nomMarque = ?2 ", Produit.class);
